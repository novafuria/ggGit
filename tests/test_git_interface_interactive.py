...
class TestGitInterfaceInteractive:
        ...
        with patch.object(git, "is_git_repository", return_value=True):
            with patch.object(git, "get_current_branch", return_value="main"):
                with patch.object(
                    git,
                    "get_branches",
                    return_value=["main", "feature/test", "develop"],
                ):
        ...
        with patch.object(git, "is_git_repository", return_value=True):
            with patch.object(git, "get_current_branch", return_value="main"):
                with patch.object(
                    git,
                    "get_branches",
                    return_value=["main", "feature/test", "origin/develop"],
                ):
                    result = git.get_mergeable_branches()
        ...
        with patch.object(git, "is_git_repository", return_value=True):
            with patch("subprocess.run") as mock_run:
                ...
                mock_run.side_effect = [
                    ...
                    MagicMock(
                        returncode=0, stdout="* feature/test abc123 Last commit message"
                    ),
                ]
    ...
    def test_is_branch_mergeable_success(self):
        ...
        with patch.object(git, "is_git_repository", return_value=True):
            with patch.object(
                git, "get_branches", return_value=["main", "feature/test"]
            ):
                with patch.object(git, "get_current_branch", return_value="main"):
    ...
    def test_is_branch_mergeable_remote_branch(self):
        ...
        with patch.object(git, "is_git_repository", return_value=True):
            with patch.object(
                git, "get_branches", return_value=["main", "origin/develop"]
            ):
                with patch.object(git, "get_current_branch", return_value="main"):
    ...
    def test_is_branch_mergeable_subprocess_error(self):
        ...
        with patch.object(git, "is_git_repository", return_value=True):
            with patch.object(
                git, "get_branches", side_effect=subprocess.CalledProcessError(1, "git")
            ):
                with pytest.raises(GitCommandError, match="Git command failed"):
                    git.is_branch_mergeable("feature/test")
