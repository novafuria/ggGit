...
class TestGetConfig:
    ...
    def test_get_config_existing_key(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch("pathlib.Path.home", return_value=Path(tmpdir)):
                # Create default config
                default_config = {"ui": {"colors": {"success": "green"}}}
                default_path = Path(tmpdir) / ".gggit" / "default-config.yaml"
                default_path.parent.mkdir(parents=True, exist_ok=True)
                with open(default_path, "w") as f:
                    yaml.dump(default_config, f)
    ...
    def test_deep_merge(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch("pathlib.Path.home", return_value=Path(tmpdir)):
                ...
                base = {"ui": {"colors": {"success": "green", "error": "red"}}}

                override = {"ui": {"colors": {"success": "bright_green"}}}
    ...
    def test_get_config_path_for_level(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch("pathlib.Path.home", return_value=Path(tmpdir)):
                ...
                assert "default-config.yaml" in config._get_config_path_for_level(
                    "default"
                )
    ...
    def test_full_configuration_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch("pathlib.Path.home", return_value=Path(tmpdir)):
                ...
                assert config.get_config("ui.verbose") == True
                assert (
                    config.get_config("ui.colors.error") == "red"
                )  # Still from default
