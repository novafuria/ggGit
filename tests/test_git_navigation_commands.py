        ...
        if command_name == "ggmain":
            with patch.object(cmd.git, "is_git_repository", return_value=True):
                with patch.object(cmd.git, "switch_branch", return_value=True):
                    with patch(
                        "src.commands.ggmain.ColorManager.success"
                    ) as mock_success:
                        result = cmd.execute()
                        assert result == 0
            ...
            with patch.object(cmd.git, "is_git_repository", return_value=True):
                with patch.object(cmd.git, "switch_branch", return_value=True):
                    with patch(
                        "src.commands.ggdevelop.ColorManager.success"
                    ) as mock_success:
                        result = cmd.execute()
                        assert result == 0
            ...
            with patch.object(cmd.git, "is_git_repository", return_value=True):
                with patch.object(
                    cmd.git,
                    "get_all_branches",
                    return_value={"local": ["main"], "remote": []},
                ):
                    with patch.object(
                        cmd.git, "get_current_branch", return_value="main"
                    ):
                        with patch.object(
                            cmd, "_show_branch_options", return_value=0
                        ) as mock_display:
                            result = cmd.execute()
                            assert result == 0
        ...
        if command_name in ["ggmain", "ggdevelop"]:
            with patch.object(cmd.git, "is_git_repository", return_value=True):
                with patch.object(cmd.git, "switch_branch", return_value=False):
                    with patch(
                        f"src.commands.{command_name}.ColorManager.error"
                    ) as mock_error:
                        result = cmd.execute()
                        assert result == 1
                        if command_name == "ggmain":
                            mock_error.assert_called_once_with(
                                "Error al cambiar a rama main"
                            )
                        else:
                            mock_error.assert_called_once_with(
...
class TestGitNavigationCommandsSpecific:
    ...
    def test_ggb_display_branches(self):
        ...
        with patch("click.echo") as mock_echo:
            ...
            )  # At least 5 calls for the branches and headers
    ...
    def test_ggb_display_branches_empty(self):
        ...
        with patch("click.echo") as mock_echo:
            ...
            assert (
                mock_echo.call_count >= 2
            )  # At least 2 calls for the warning messages
