        ...
        with patch.object(git, "is_git_repository", return_value=True):
            with patch("subprocess.run") as mock_run:
                ...
                mock_run.assert_called_once_with(
                    ["git", "diff"], capture_output=False, text=True
                )
    ...
    def test_diff_staged(self):
        ...
        with patch.object(git, "is_git_repository", return_value=True):
            with patch("subprocess.run") as mock_run:
                ...
                assert result is True
                mock_run.assert_called_once_with(
                    ["git", "diff", "--staged"], capture_output=False, text=True
                )
    ...
    def test_unstage_files_basic(self):
        ...
        with patch.object(git, "is_git_repository", return_value=True):
            with patch("subprocess.run") as mock_run:
                ...
                assert result is True
                mock_run.assert_called_once_with(
                    ["git", "reset", "HEAD"], capture_output=True, text=True
                )
    ...
    def test_reset_hard(self):
        ...
        with patch.object(git, "is_git_repository", return_value=True):
            with patch("subprocess.run") as mock_run:
                ...
                assert result is True
                mock_run.assert_called_once_with(
                    ["git", "reset", "--hard", "HEAD"], capture_output=True, text=True
                )
