...
class TestGgaiCommand:
    ...
    def test_execute_main_not_git_repo(self):
        ...
        with patch.object(self.command, "_is_ai_configured", return_value=True):
            with patch.object(
                self.command.git, "is_git_repository", return_value=False
            ):
                result = self.command.execute_main()
    ...
    def test_execute_main_should_use_ai(self):
        ...
        with patch.object(self.command, "_is_ai_configured", return_value=True):
            with patch.object(self.command.git, "is_git_repository", return_value=True):
                with patch.object(
                    self.command.analyzer,
                    "should_use_ai",
                    return_value=(True, {"files": ["test.py"], "has_staged": True}),
                ):
                    with patch.object(
                        self.command.git,
                        "get_diff_content",
                        ...
                        with patch.object(
                            self.command.message_generator,
                            "generate_message",
                            return_value="feat: test message",
                        ):
                            with patch.object(
                                self.command.analyzer,
        ...
        with patch.object(self.command, "_is_ai_configured", return_value=True):
            with patch.object(self.command.git, "is_git_repository", return_value=True):
                with patch.object(
                    self.command.analyzer,
                    "should_use_ai",
                    return_value=(False, {"files": ["test.py"], "has_staged": True}),
                ):
                    with patch.object(
                        self.command.analyzer,
    ...
    def test_execute_usage_tracking_disabled(self):
        ...
        with patch.object(
            self.command.usage_tracker, "is_tracking_enabled", return_value=False
        ):
    ...
    def test_execute_usage_tracking_enabled(self):
        ...
        with patch.object(
            self.command.usage_tracker, "is_tracking_enabled", return_value=True
        ):
            with patch.object(
                self.command.usage_tracker,
                "get_usage_stats",
                return_value={
    ...
    def test_execute_test_connection_success(self):
        ...
        with patch.object(
            self.command.message_generator, "test_connection", return_value=True
        ):
            with patch.object(
                self.command.message_generator,
                "get_provider_info",
    ...
    def test_execute_test_connection_failure(self):
        ...
        with patch.object(
            self.command.message_generator, "test_connection", return_value=False
        ):
            result = self.command.execute_test()
