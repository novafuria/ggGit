    ...
    def test_get_config_level_user(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch("pathlib.Path.home", return_value=Path(tmpdir)):
                ...
                user_config = {"ui": {"colors": {"success": "bright_green"}}}
    ...
    def test_list_config_keys_specific_level(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch("pathlib.Path.home", return_value=Path(tmpdir)):
                ...
                user_config = {
                    "ai": {"enabled": True},
                    "ui": {"colors": {"success": "bright_green"}},
                }
                user_path = Path(tmpdir) / ".gggit" / "user-config.yaml"
    ...
    def test_key_exists_in_config(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch("pathlib.Path.home", return_value=Path(tmpdir)):
                ...
                test_config = {"ui": {"colors": {"success": "green"}}}

                # Test existing key
                assert (
                    config._key_exists_in_config(test_config, "ui.colors.success")
                    is True
                )
                ...
                assert (
                    config._key_exists_in_config(test_config, "ui.colors.error")
                    is False
                )
                assert (
                    config._key_exists_in_config(test_config, "nonexistent.key")
                    is False
                )
    ...
    def test_full_advanced_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch("pathlib.Path.home", return_value=Path(tmpdir)):
                ...
                default_config = {
                    "version": "1.0",
                    "ui": {"colors": {"success": "green", "error": "red"}},
                }
                default_path = Path(tmpdir) / ".gggit" / "default-config.yaml"
                default_path.parent.mkdir(parents=True, exist_ok=True)
                with open(default_path, "w") as f:
