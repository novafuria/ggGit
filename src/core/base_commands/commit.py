...
class CommitCommand(BaseCommand):
    ...
    def execute(
        self, message: str, scope: Optional[str] = None, amend: bool = False
    ) -> int:
        """
        ...
        try:
            ...
            if not self.git.is_git_repository():
                self.logger.log_error(
                    Exception("No es un repositorio Git válido"), "execute"
                )
                return 1
            ...
            if not amend:
                ...
                if not staged_files and not unstaged_files:
                    self.logger.log_error(
                        Exception("No hay cambios para hacer commit"), "execute"
                    )
                    return 1
                ...
                if unstaged_files:
                    if not self.git.stage_all_changes():
                        self.logger.log_error(
                            Exception("Error al stagear cambios"), "execute"
                        )
                        return 1
            ...
            if amend:
                if not self._execute_amend_commit(commit_message):
                    self.logger.log_error(
                        Exception("Error al realizar commit --amend"), "execute"
                    )
                    return 1
            else:
                if not self.git.commit(commit_message):
                    self.logger.log_error(
                        Exception("Error al realizar commit"), "execute"
                    )
                    return 1
    ...
    def _execute_amend_commit(self, message: str) -> bool:
        ...
        try:
            ...
            self.logger.log_error(
                Exception("Timeout al ejecutar 'git commit --amend'"),
                "_execute_amend_commit",
            )
            return False
        except subprocess.CalledProcessError as e:
            self.logger.log_error(
                Exception(
                    f"Error al ejecutar 'git commit --amend' (código {e.returncode}): {e.stderr}"
                ),
                "_execute_amend_commit",
            )
            return False
