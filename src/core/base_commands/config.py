...
class ConfigCommand(BaseCommand):
        ...
        try:
            if action == "get":
                ...
                self.logger.log_error(
                    ValueError(f"Unknown action: {action}"), "execute"
                )
                return 1
    ...
    def _execute_get(self, key: Optional[str]) -> int:
        ...
        if not key:
            self.logger.log_error(
                ValueError("Key is required for get action"), "_execute_get"
            )
            return 1
    ...
    def _execute_set(self, key: Optional[str], value: Optional[str], level: str) -> int:
        ...
        if not key:
            self.logger.log_error(
                ValueError("Key is required for set action"), "_execute_set"
            )
            return 1

        if value is None:
            self.logger.log_error(
                ValueError("Value is required for set action"), "_execute_set"
            )
            return 1
