...
CONVENTIONAL_COMMIT_PREFIXES = (
    r"^(feat|fix|docs|style|refactor|test|chore|perf|ci|build|break)(\([^)]+\))?:\s*"
)
...
class AiMessageGenerator:
    ...
    def _analyze_change_type(self, diff_content: str) -> str:
        ...
        if any(
            keyword in diff_lower for keyword in ["add", "new", "create", "implement"]
        ):
            return "feature"
            ...
            keyword in diff_lower for keyword in ["refactor", "clean", "optimize"]
        ):
            return "refactor"
        else:
            return "chore"
    ...
    def _get_additional_context(self, files: List[str], diff_content: str) -> str:
        ...
        if len(files) > 5:
            context_parts.append(
                "Note: Multiple files modified - focus on the main change"
            )
        ...
        config_files = [
            f
            for f in files
            if any(ext in f for ext in [".yml", ".yaml", ".json", ".toml"])
        ]
        if config_files:
            context_parts.append(
                f"Note: Configuration files modified: {', '.join(config_files)}"
            )
        ...
        if doc_files:
            context_parts.append(
                f"Note: Documentation files modified: {', '.join(doc_files)}"
            )
