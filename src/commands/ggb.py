            ...
            if self._branch_exists(clean_name):
                click.echo(
                    ColorManager.warning(
                        f"Rama {clean_name} ya existe, cambiando a ella"
                    )
                )
                try:
                    ...
                    if result:
                        click.echo(
                            ColorManager.success(f"Cambiado a rama '{clean_name}'")
                        )
                        return 0
                    else:
                        click.echo(
                            ColorManager.error(
                                f"Error al cambiar a rama '{clean_name}'"
                            )
                        )
                        return 1
                    ...
                    if (
                        "would be overwritten" in error_msg
                        or "local changes" in error_msg
                    ):
                        click.echo(
                            ColorManager.warning(
                                "Tienes cambios sin commitear que serían sobrescritos"
                            )
                        )
                        ...
                        click.echo(
                            ColorManager.error(
                                f"Error al cambiar a rama '{clean_name}': {error_msg}"
                            )
                        )
                        return 1
            else:
                    ...
                    if result:
                        ...
                        try:
                            ...
                            if switch_result:
                                click.echo(
                                    ColorManager.success(
                                        f"Rama '{clean_name}' creada y cambiado a ella"
                                    )
                                )
                                return 0
                            else:
                                click.echo(
                                    ColorManager.warning(
                                        f"Rama '{clean_name}' creada pero no se pudo cambiar a ella"
                                    )
                                )
                                click.echo(
                                    ColorManager.info(
                                        f"Puedes cambiar manualmente con: git checkout {clean_name}"
                                    )
                                )
                                return 0
                        except subprocess.CalledProcessError as e:
                            ...
                            if (
                                ...
                                click.echo(
                                    ColorManager.warning(
                                        f"Rama '{clean_name}' creada pero no se pudo cambiar"
                                    )
                                )
                                click.echo(
                                    ColorManager.warning(
                                        "Tienes cambios que serían sobrescritos"
                                    )
                                )
                                click.echo(ColorManager.info("Opciones:"))
                                click.echo(
                                ...
                                click.echo(
                                    f"  4. Cambiar manualmente después: git checkout {clean_name}"
                                )
                                return 0
                            else:
                                click.echo(
                        ...
                        click.echo(
                            ColorManager.error(f"Error al crear rama '{clean_name}'")
                        )
                        return 1
                except subprocess.CalledProcessError as e:
                    click.echo(
                        ColorManager.error(
                            f"Error al crear rama '{clean_name}': {e.stderr}"
                        )
                    )
                    return 1
            ...
            while True:
                try:
                    ...
                    if choice == "1":
                        ...
                        if result.returncode == 0:
                            click.echo(
                                ColorManager.success("Cambios guardados en stash")
                            )
                            return True
                        else:
                            click.echo(
                                ColorManager.error(
                                    f"Error al guardar cambios: {result.stderr}"
                                )
                            )
                            return False
                        ...
                        if result1.returncode == 0 and result2.returncode == 0:
                            ...
                            click.echo(
                                ColorManager.error(
                                    f"Error al descartar cambios: {result1.stderr or result2.stderr}"
                                )
                            )
                            return False
                        ...
                        click.echo(
                            ColorManager.error("Selección inválida. Ingresa 1, 2 o 3")
                        )
        ...
        try:
            ...
            while True:
                try:
                    ...
                    click.echo(
                        ColorManager.error(
                            "Selección inválida. Ingresa un número válido"
                        )
                    )
                except KeyboardInterrupt:
                    click.echo(ColorManager.warning("\nOperación cancelada"))
                ...
                try:
                    ...
                    if 1 <= choice_num <= len(branches):
                        ...
                        if result:
                            click.echo(
                                ColorManager.success(
                                    f"Cambiado a rama {selected_branch}"
                                )
                            )
                            return 0
                        else:
                            click.echo(
                                ColorManager.error(
                                    f"Error al cambiar a rama {selected_branch}"
                                )
                            )
                            return 1
                    else:
                        click.echo(
                            ColorManager.error(
                                f"Selección inválida. Ingresa un número entre 1 y {len(branches)}"
                            )
                        )
