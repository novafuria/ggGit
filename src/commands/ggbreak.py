    ...
    def execute(self, message, scope=None, ai=False, amend=False):
        ...
        try:
            ...
            if not message:
                ...
                click.echo(
                    ColorManager.info(
                        "O proporciona un mensaje manual: ggbreak 'mensaje'"
                    )
                )
                return 1
...
@click.command()
@click.option(
    "--amend", "-a", is_flag=True, help="Amend the last commit with the new message"
)
@click.option(
    "--scope", "-s", help="Add a scope to the prefix (e.g. break(scope): message)"
)
